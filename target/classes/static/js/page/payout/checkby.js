define(['/js/lib/juicer.js', '/js/common/ajax.js', '/js/common/page.js', '/js/common/autoComplete.js'], function(juicer, req, pagenation, autoCmp) {
    ...
    function initSearch(){
        ...
        var url = "/admin/allowance/compensate/pass";
    ...
    function toPay(){
        ...
        var curTr = parent.parent();
        var paymentDate = curTr.find('.payment-date');
        var paymentState = curTr.find('.pay-state');
        var allowanceTime = curTr.find('.allowance-time');

        var param = {
            "applyId": parent.data("applyid"),
            "allowanceApplyId": parent.data("allowanceapplyid"),
            "allowanceSubType": parent.data("allowancesubtype")
        };

        req({
            type: 'post',
            url: "/admin/allowance/compensate/dopay",
            data: param
        }).done(function(res){
            if(res.result.code == "500000"){
                ...
                var resPaymentState
                var resPaymentDate = res.data.allowance.paymentDate;
                
                resPaymentState = res.data.allowance.status;
                if(resPaymentState == 0){
                    resPaymentState = "新建"
                }else if(resPaymentState == 1){
                    resPaymentState = "待付"
                }else if(resPaymentState == 2){
                    resPaymentState = "付款请求发送"
                }else if(resPaymentState == 3){
                    resPaymentState = "付款失败"
                }else if(resPaymentState == 4){
                    resPaymentState = "付款成功"
                }else if(resPaymentState == 6){
                    resPaymentState = "付款异常"
                }
                paymentDate.text(resPaymentDate);
                paymentState.text(resPaymentState);
                allowanceTime.text(resPaymentDate);
                _this.text('已支付')
                    .attr('disabled', true);
                alert(res.result.message);
            }
