<script id="template" type="text/html">
	{@each allownaceList as allownace}
		<tr>
            ...
            <td>
                {@if allownace.paymentReqTime==null}

                {@else}
                    {{allownace.paymentReqTime}}
                {@/if}
            </td>
            <td>
                {@if allownace.paymentConfirmTime==null}

                {@else}
                    {{allownace.paymentConfirmTime}}
                {@/if}
            </td>
            <td>
            ...
            <td>
                {@if allownace.status==4}
                    <a href="http://mpay.sankuai.com/mis/funds/query?partnerCode=smart-pos&init=false&outNo=&payTaskId=&merchantNo={{allownace.paymentNo}}" class="to-detail">
                        查看回单
                    </a>&nbsp&nbsp
                {@/if}
